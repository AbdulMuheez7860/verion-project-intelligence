...
import type { SecurityFreshness, SecurityPosture } from '@/types/api'
...
interface SecurityPostureHeaderProps {
  posture: SecurityPosture
  score?: number | null
  freshness: SecurityFreshness
}

export function SecurityPostureHeader({ posture, score, freshness }: SecurityPostureHeaderProps) {
        ...
        >
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Security posture</p>
              <div className="mt-1 flex flex-wrap items-center gap-2">
                <h2 className="text-lg font-semibold tracking-tight">{posture.label}</h2>
                {score != null ? (
                  <Badge tone={postureTone(posture.level)}>Score {Math.round(score)}</Badge>
                ) : (
          ...
          <div
            className="shrink-0 rounded-lg border border-border bg-card px-3 py-2 text-sm"
            role="status"
            aria-live="polite"
          >
