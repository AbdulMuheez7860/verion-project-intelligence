...
interface CodeQualitySeverityDistributionProps {
  counts?: SeverityCounts | null
  hasData: boolean
  total?: number
}

export function CodeQualitySeverityDistribution({
  counts,
  hasData,
  total,
}: CodeQualitySeverityDistributionProps) {
  const computedTotal =
      ...
      <CardContent>
        {!hasData || !counts ? (
          <EmptyState
            ...
            description="Severity distribution appears after code quality analysis completes."
            className="min-h-28"
          />
        ) : (
          <div>
            <p className="sr-only">
              Quality findings distribution:{' '}
            ...
            <p className="mt-4 text-metadata">{computedTotal} total quality findings</p>
          </div>
        )}
      </CardContent>
    </Card>
