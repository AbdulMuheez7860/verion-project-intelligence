...
interface DependencySeverityDistributionProps {
  counts?: SeverityCounts | null
  hasData: boolean
  total?: number
}

export function DependencySeverityDistribution({
  counts,
  hasData,
  total,
}: DependencySeverityDistributionProps) {
  const computedTotal =
    total ?? (counts ? counts.critical + counts.high + counts.medium + counts.low : 0)
      ...
      <CardContent>
        {!hasData || !counts ? (
          <EmptyState
            ...
            description="Severity distribution appears after dependency vulnerability analysis completes."
            className="min-h-28"
          />
        ) : (
          <div>
            <p className="sr-only">
              Dependency vulnerability distribution:{' '}
            ...
            <p className="mt-4 text-metadata">{computedTotal} total vulnerability findings</p>
          </div>
        )}
      </CardContent>
    </Card>
